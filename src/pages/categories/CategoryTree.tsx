...
const CategoryTree = ({ data }: Props): JSX.Element => {
  ...
  const highlight = (text: string) => {
    ...
    return (
      <span className="text-zinc-900">
        {before}
        <span className="rounded-md bg-yellow-100 px-1 font-semibold">{match}</span>
        {after}
      </span>
    );
  };
        ...
        {nodes.map((node) => {
          ...
          return (
            <li key={node._id}>
              <div
                className={[
                  "group flex items-center gap-2 rounded-2xl border border-black/10 bg-white px-3 py-2",
                ...
                <button
                  ...
                  className={[
                    "h-7 w-7 rounded-xl grid place-items-center border border-black/10 bg-white",
                    hasChildren ? "hover:bg-zinc-50" : "opacity-40 cursor-default",
                  ].join(" ")}
                ...
                {hasChildren ? (
                  <button
                    ...
                    className="text-xs font-semibold text-zinc-700 hover:text-zinc-900 transition">
                    {isOpen
                      ? language === "ar"
                        ? "طي"
                        : "Close"
  ...
  return (
    <div className="rounded-3xl border border-black/10 bg-white/80 backdrop-blur shadow-sm p-5">
      {/* Header */}
      ...
      <div className="flex items-center gap-2">
        <div className="relative w-full">
          ...
          <input
            ...
            className="w-full border border-black/10 bg-white rounded-2xl py-2.5 pl-10 pr-10 text-sm outline-none focus:ring-2 focus:ring-blue-500"
          />
          {query ? (
            <button
              type="button"
      ...
      <div className="mt-4 max-h-[420px] overflow-y-auto pr-1">
        {filtered?.length > 0 ? (
          ...
          <div className="rounded-2xl border border-black/10 bg-white p-6 text-center text-zinc-500">
            {t.empty}
          </div>
        )}
      </div>
